...
class EventFactory {
  ...
  /**
   * a bufer for the last table selection event, needed to filter superfluous events generated by the winbinder library
   *
   * @var array
   */
  private static $lastTableSelectionEvent = null;
  ...
  public static function createEvent(EventInfo $eventInfo) {
    ...
    if(!($eventInfo->source instanceof Table) && self::$lastTableSelectionEvent  instanceof ActionEvent) {
      $events[] = self::$lastTableSelectionEvent ;
      self::$lastTableSelectionEvent  = new NoOpEvent($eventInfo);
    }
  ...
  private static function createActionEvent($eventInfo) {
    ...
    if($event->getSource() instanceof Table) {
      self::$lastTableSelectionEvent  = $event;
